...
亮度范围 -150% ~ 100%: 1~100 写系统亮度; ≤0 时系统亮度压到下限、
遮罩逐级加深 (0~-100% 约 92% 黑度, -100~-150% 加深到约 98% 黑度),
远低于系统最低亮度 (深夜全黑房间可用, OLED 屏效果尤佳)。
...
通道被系统关闭 (APK 被替换 / ROM 清理 / 电池优化) 时, 应用会**每 5 秒自动检测并恢复**,
不需要手动去设置里重新打开 (依赖 root 脚本 `pm grant WRITE_SECURE_SETTINGS`)。

### 超暗区间 -150%
亮度范围 -150% ~ 100%。系统亮度被压到最低后, 透明黑色遮罩逐级加深:
0 ~ -100% 约 92% 黑度, 继续下滑到 -150% 加深至约 **98% 黑度**,
远低于系统最低亮度, 深夜全黑房间可用, OLED 屏效果尤佳。
边缘继续下滑即可一路压到近乎全黑。

> 系统亮度本身已到面板硬件下限 (如本机 `brightnessMin=4 / 2047`),
> 所以 -100% 之后能继续压暗的只有遮罩黑度, -150% 已是可视化极限。
